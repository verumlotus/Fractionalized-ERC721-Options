...
contract FractionalizedOption {
    ...
    IPrimitiveFactory constant primFactory =
        IPrimitiveFactory(0x5cA2D631a37B21E5de2BcB0CbB892D723A96b068);
    ...
    IFractionalVaultFactory constant fractionalArtFactory =
        IFractionalVaultFactory(0x85Aa7f78BdB2DE8F3e0c0010d99AD5853fFcfC63);
    ...
    constructor(
        address _stable,
        FractionalParams memory _fractionalParams,
        PrimitivePoolParams memory _primitivePoolParams
    ) {
        owner = msg.sender;
        ...
        uint256 vaultId = fractionalArtFactory.mint(
            _fractionalParams.name,
            _fractionalParams.symbol,
            _token,
            _tokenId,
            1e36,
            _fractionalParams.listPrice,
            _fractionalParams.fee
        );

        fractionalArtVault = IFractionalVault(
            fractionalArtFactory.vaults(vaultId)
        );
        fractionalArtVault.updateCurator(msg.sender);

        // Primitive Finance Logic
        // Create the engine
        engine = IPrimitiveEngine(
            primFactory.deploy(address(fractionalArtVault), address(stable))
        );
        ...
        fractionalArtVault.safeTransfer(
            msg.sender,
            fractionalArtVault.balanceOf(address(this))
        );
     ...
     */
    function depositLiquidity(uint256 delRisky, uint256 delStable) external {
        fractionalArtVault.safeTransferFrom(
            msg.sender,
            address(this),
            delRisky
        );
     ...
     */
    function withdrawInitialLiquidity(uint256 delRisky, uint256 delStable)
        external
        onlyOwner
    {
